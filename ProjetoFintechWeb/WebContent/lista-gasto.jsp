...
<title>Listagem Gasto</title>
</head>
<body>
	<div class="container">
        <div class="tittle-entidades"><p>Listagem Gasto</p></div>


		<div class="listagem-entidades">
            <p>Gasto no período</p>
		...
		<table class="table table-striped">
			...
			<c:forEach items="${gastos}" var="g">
				<tr>
					...
					<td>${g.descricao}</td>
					<td>
						<fmt:formatDate value="${g.dtGasto.time }" pattern="dd/MM/yyyy"/>
					</td>
					<td>${g.categoria}</td>
					<td>
						<c:url value="gasto" var="link">
							<c:param name="acao" value="abrir-form-edicao"/>
							<c:param name="codigoGasto" value="${g.codigoGasto}"/>
						</c:url>
						<a class="editar" href="${link}">Editar</a>
						<button type="button" class="excluir" data-toggle="modal" data-target="#excluirModal" onclick="codigoExcluir.value = ${g.codigoGasto}">
  							Excluir
						</button>
					</td>
				</tr>
				...
				<div class="modal-content">
					...
					<div class="modal-footer">
						<form action="gasto" method="post">
							...
							<input type="hidden" name="codigoGasto" id="codigoExcluir">
